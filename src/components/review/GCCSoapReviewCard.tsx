...
export default function GCCSoapReviewCard({
  ...
  const [chiefComplaint, setChiefComplaint] = useState("Chief Complaint");

  useEffect(() => {
    if (!sessionId) {
      setNarrative(gccSoapReviewData.patientNarrative);
      setSymptoms([...gccSoapReviewData.symptoms]);
      setHistory(gccSoapReviewData.history);
      setChiefComplaint("Chief Complaint");
      return;
    }

    if (!soapNote) return;
    setNarrative(soapNote.subjective.patient_narrative || "Requires clinician review");
    setSymptoms(soapNote.subjective.symptoms?.length ? soapNote.subjective.symptoms : ["Requires clinician review"]);
    setHistory(soapNote.subjective.history || "Requires clinician review");
    setChiefComplaint(soapNote.subjective.chief_complaint || "Chief Complaint");
  }, [sessionId, soapNote]);
  ...
  return (
    <GCCReviewCardFrame title="Subjective Assessment" isEditing={isEditing} onEditToggle={() => setIsEditing((value) => !value)} minHeightClass="min-h-[650px]">
      {sessionId && !soapNote ? (
        <div className="rounded-[16px] border border-[#D8DDF2] bg-white/80 px-4 py-5 text-[14px] font-semibold text-[#101BD8]">
          {isLoading ? "Loading generated SOAP Notes..." : errorMessage ?? "Generated SOAP Notes are not available for this session yet."}
        </div>
      ) : (
        <div className="space-y-6">
          <section>
            <h3 className="text-[18px] font-semibold leading-6 text-[#080B3A]">Patient Narrative</h3>
            <p className="mt-4 text-[13px] font-semibold uppercase tracking-[0.06em] text-[#697085]">{chiefComplaint}</p>
            {isEditing ? (
              <textarea
                value={narrative}
